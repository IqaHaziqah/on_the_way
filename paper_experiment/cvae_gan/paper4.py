...
from cvaegan_v8 import VAE_GAN
import copy
import logging
...
def generate(N,data,label,para_o):
    ...
    for i in range(N):
        ...
        if PRE != None:
            min_max_scaler = PRE(feature_range=(-1,1))
...
para_o = {
    ...
    'epochs':1000,
    'batch_size':20, #为偶数
    'rate_1':0.005,#两个分类型网络的学习率
    'rate_2':0.005,#generator的学习率
    'PRE':sklearn.preprocessing.MinMaxScaler,#do not change it
#    'PRE':None,
    'hidden':[20,10],   #分别是指编码器中的hidden1，hidden2，分类器中的hidden1，hidden2
    'lamda':[1,1,0.01,0.01,0.1],
