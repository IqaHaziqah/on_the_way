...
class VAE_GAN(object):
    def __init__(self, sess,para_o):
        ...
        self.rp = tf.placeholder('float32',shape=[None,self.input_dim])
        self.rn = tf.placeholder('float32',shape=[None,self.input_dim])
    ...
    def linear(self,x,shape,scope):
        w = tf.get_variable(initializer=tf.random_uniform(shape,minval=-np.sqrt(2/shape[0]),maxval=np.sqrt(2/shape[0]),seed=31),name=scope+'_w')
...
#        LC = -tf.reduce_sum(2*self.y_pred[0]*self.label)/(tf.reduce_sum(tf.square(self.y_pred[0]))+tf.reduce_sum(tf.square(self.label)))
        ...
        '''真实的z'''
        z = self.mu + tf.exp(self.sigma) * tf.random_normal(tf.shape(self.mu), 0, 1, dtype=tf.float32)
        '''KL = tf.reduce_mean(-0.5 * tf.reduce_sum(
            tf.square(self.mu) + tf.square(self.sigma) - tf.log(1e-8 + tf.square(self.sigma)) - 1,
            [1]))
        '''
        KL = tf.reduce_mean(-0.5*tf.reduce_sum(1+self.sigma-tf.square(self.mu)-tf.exp(self.sigma),axis=-1))
        '''xf为原始z生成的样本'''
        xf = self.generator(label=self.label,z=z)
        zp = self.z
        cp = np.concatenate((np.ones([int(self.batch_size/2),1]),np.zeros([int(self.batch_size/2),1])))
        '''xp为采样后生成的样本'''
        '''这部分为分辨器中的结果和损失函数'''
...
#        loss_fake_p = tf.reduce_sum(tf.square(dxp[0]-y_neg))
        ...
        _,_,xcrp = self.classifier(self.rp)
        _,_,xcrn = self.classifier(self.rn)
        _,_,xcr = self.classifier(self.image)
        _,_,xcf = self.classifier(xf)
        ans = self.feature_center(cp,xcp)
        self.posp += ans[0]
        self.negp += ans[1]
        self.posr += tf.reduce_mean(xcrp)
        self.negr += tf.reduce_mean(xcrn)
        self.LGC = 0.5*(tf.reduce_sum(tf.square(self.posp-self.posr)+tf.square(self.negp-self.negr),axis=0))
...
#                batch_z = tf.cast(batch_z,tf.float32)
                ...
                _,  d_loss = self.sess.run([self.d_optim,  self.d_loss],
                                        feed_dict={self.image:batch_images,self.z: batch_z,self.label:batch_labels})
                _,  c_loss = self.sess.run([self.cla_optim,  self.c_loss],
                                        feed_dict={self.image:batch_images,self.z: batch_z,self.label:batch_labels})
                # update decoder network
                _,dec_loss,lgc,lgd = self.sess.run([self.dec_optim,self.dec_loss,self.LGC,self.LGD],
                                                   feed_dict={self.image:batch_images,self.rn:batch_images[np.squeeze(batch_labels)==0],
...
#                          % (epoch, idx, self.num_batches, time.time() - start_time, d_loss, dec_loss, enc_loss))
    ...
    def oversampling(self,ovsize):
        ...
        cp = np.random.randint(1,2,size=[ovsize,1])
