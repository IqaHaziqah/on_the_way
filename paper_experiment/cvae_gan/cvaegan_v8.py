...
class VAE_GAN(object):
    def __init__(self, sess,para_o):
        ...
        self.c_dim = 1 
        self.learning_rate = para_o['rate_1']
        self.learning_rate_ge = para_o['rate_2']
        self.r = 0.05
        self.beta1 = 0.5
    ...
    def linear(self,x,shape,scope):
        b = 1.0*np.sqrt(6.0/(shape[0]+shape[1]))
        ini = np.random.uniform(low=-b,high=b,size=shape)
        w = tf.get_variable(initializer=ini,name=scope+'_w',dtype=tf.float64,regularizer=self.regularizer)
    ...
    def encoder(self,x,reuse=tf.AUTO_REUSE):
        with tf.variable_scope("encoder", reuse=reuse):
            ...
            stddev = gaussian_params[:,self.z_dim:]
        return mean, stddev
    ...
    def discriminator(self, x, reuse=tf.AUTO_REUSE):
        ...
        with tf.variable_scope("discriminator", reuse=reuse):
            ...
            out_logit = self.linear(net,shape=[self.hidden1,1], scope='d_fc2')
    ...
    def classifier(self,x,reuse=tf.AUTO_REUSE):
        with tf.variable_scope('classifier',reuse=reuse):
            ...
            out_logit = self.linear(net,shape=[self.hidden1,1], scope='c_fc2')
            out = tf.nn.sigmoid(out_logit)
...
#        LC = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=self.label,logits=self.y_pred))
        LF = tf.reduce_mean(tf.reduce_sum(-tf.log(1e-8+2*self.y_pred[0]*self.label/(self.y_pred[0]**2+self.label**2)),reduction_indices=1))
...
#        LC = -tf.reduce_sum(2*self.y_pred[0]*self.label)/(tf.reduce_sum(tf.square(self.y_pred[0]))+tf.reduce_sum(tf.square(self.label)))
        ...
        y_neg = K.zeros_like(self.label)*0.1
...
#        print(tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES))
        ...
        self.d_loss_1 = LD1 #+tf.reduce_mean(regularization_loss)
        self.d_loss_2 = LD2
        self.dec_loss = self.lamda2*LG + self.lamda3*self.LGD + self.lamda4*self.LGC #+tf.reduce_mean(regularization_loss)
        self.c_loss = LC+self.lamda5*LF#+tf.reduce_mean(regularization_loss)
        self.enc_loss = self.lamda1*KL + self.lamda2*LG       # +tf.reduce_mean(regularization_loss)
        
        self.g = tf.gradients(self.dec_loss,dec_vars)
        ...
        with tf.control_dependencies(tf.get_collection(tf.GraphKeys.UPDATE_OPS)):
            ...
            self.d_optim1 = tf.train.AdamOptimizer(self.learning_rate) \
                      .minimize(self.d_loss_1, var_list=d_vars)
            self.d_optim2 = tf.train.AdamOptimizer(self.learning_rate) \
                      .minimize(self.d_loss_2, var_list=d_vars)
            
            self.dec_optim = tf.train.AdamOptimizer(self.learning_rate_ge) \
                      .minimize(self.dec_loss, var_list=dec_vars)
            self.enc_optim = tf.train.AdamOptimizer(self.learning_rate_ge) \
...
#                    self.graident_check(feed)
                _,  d_loss = self.sess.run([self.d_optim1,  self.d_loss_1],
                                        feed_dict=feed)
                _,  d_loss = self.sess.run([self.d_optim2,  self.d_loss_2],
                                        feed_dict=feed)
...
#                    print(ff[0])
    ...
    def pretrain(self,args):
        data,label = args
        me = [self.lamda3,self.lamda4,self.lamda5]
        self.lamda3,self.lamda4,self.lamda5 = [0,0,0]
        pos = data[label==1]
        n = pos.shape[0]
        pos = Dataset(pos)
        self.num_batches = int(n/self.batch_size)
        for epoch in range(self.epoch):
            for idx in range(self.num_batches):
                batch_images = pos.next_batch(self.batch_size)[0]
                batch_labels = np.ones([self.batch_size,1])
                batch_z = np.random.normal(size=[self.batch_size,self.z_dim])
                feed = {self.image:batch_images,self.z:batch_z,self.label:batch_labels}
                _,loss1 = self.sess.run([self.dec_optim,self.dec_loss],feed_dict=feed)
                _,loss2 = self.sess.run([self.enc_optim,self.enc_loss],feed_dict=feed)
#            print('epoch[%2d],dec_loss:%.4f,enc_loss:%.4f'%(epoch,loss1,loss2))
        self.lamda3,self.lamda4,self.lamda5 = me
        return
...
#pre = model.predict(ans)
#print(pandas.value_counts(pre))
#
#model.fit(data,np.arange(0,label.shape[0]))
#pre = model.predict(ans)
#check = pandas.value_counts(pre)
#print(check.shape[0])
#
#pos = [value for value in pre if label[value]==1]
#print('distinct positive samples',len(set(pos)))
#
#pos = [value for value in pre if label[value]==0]
#print('distinct negative samples',len(set(pos)))
