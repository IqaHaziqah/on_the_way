...
def compute_gradient_comparitive_error(x,
                           x_shape,
                           y,
                           y_shape,
                           x_init_value=None,
  ...
  """Computes the gradient error. using the 相对误差
  ...
  """
  grad = tf.test.compute_gradient(x, x_shape, y, y_shape, x_init_value, delta,
  ...
  for j_t, j_n in grad:
    if j_t.size or j_n.size:  # Handle zero size tensors correctly
      shang = np.maximum(np.fabs(j_n),np.fabs(j_t))
      shang[shang==0]=1 
      error = np.maximum(error, (np.fabs(j_t - j_n)/shang).max())
  return error
def readdata(name,withlabel=False):
    #输入数据集名称，读取该数据集的mat文件，并返回data和label
...
def get_resultC(N,result,generation,name='None',write=False):
    ...
    return np.mean(ggmean,dtype='float32')
#   return np.mean(ggmean,dtype='float')
#   gF_min = []
#   ggmean = []
#   gF_maj = []
#   gacc = []
...
def get_resultNB(N,result,generation,name='None',write=False):
    ...
    print('mean F1-min:',np.mean(gF_min),'mean f-maj:',np.mean(gF_maj),'mean accuracy:',np.mean(gacc))
    print('mean gmean:',np.mean(ggmean),'mean TPrate:',np.mean(gtp),'mean AUC:',np.mean(gauc))
    b = [np.mean(gF_min),np.mean(gF_maj),np.mean(ggmean)]
    bb = [np.mean(gF_min),np.mean(gF_maj),np.mean(gacc),np.mean(ggmean),np.mean(gtp),np.mean(gauc)]
    return b,bb
#    return np.mean(ggmean,dtype='float')
#   gF_min = []
#   ggmean = []
