...
#data=np.loadtxt('..//MNIST_data//UCI//glass.txt',dtype='float32')
#for windows
...
para_o = {
    'hidden_encoder_dim':9,                                    
    'hidden_decoder_dim':9, 
    'latent_dim':2,
    'lam':0,
    'epochs':120,
    'batch_size':10,
    ...
    'norm':True,
    'initial':3,
    'RFE':sklearn.preprocessing.StandardScaler
        }
...
generation = {}
for i in range(N):
    train,train_label,_,_ = result[str(i)]
...
##    svc = svm.SVC(C=2,gamma=0.5,probability=True)
##    y_predne = svc.fit(train,train_label).predict(test)
##    y_pro = svc.predict_proba(test)[:,1]
##    temf,temg,tema = compute(test_label,y_predne,y_pro)
##    print('F1',temf,'AUC',tema,'gmean',temg)
#    gF1.append(temf)
#    ggmean.append(temg)
#    gauc.append(tema)
#print('##naive-bayes+vae#########')
...
#    data = preprocessing.minmax_scale(train)
