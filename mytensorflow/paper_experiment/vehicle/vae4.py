...
def weight_variable(shape,ini):
    ...
    if ini ==1:
        ...
        initial = np.random.randn(n,out)*np.sqrt(1.0/n)   
    elif ini == 5:
        initial = np.zeros(shape)
        b = 1.0*np.sqrt(6.0/(n+out))
        for i in range(out):
            initial[:,i]=np.random.uniform(low=-b,high=b,size=[n])
    else:
        initial = np.random.rand(n,out)*np.sqrt(2.0/n)
...
#    BCE = tf.reduce_sum(tf.abs(x_hat-x))
    BCE = input_dim*objectives.mse(y_true=x,y_pred=x_hat)
#    BCE = tf.reduce_sum(tf.square(x-x_hat),reduction_indices=1)
...
#    loss_summ = tf.summary.scalar("lowerbound", loss)
    ...
    x_hat_11 = (tf.matmul(hidden_decoder_1, W_decoder_hidden_reconstruction) + b_decoder_hidden_reconstruction)
...
#        projector.visualize_embeddings(writer,config)
        k,l=sess.run([kld,loss],feed_dict={x:data})
        
        print('lower_bound,mean_loss',k,l)
        myx = sess.run(x_hat,feed_dict={x:data})
...
#            z_sample = miu+sigma*ep
            z_sample_1 = np.random.normal(size=[gene_size,latent_dim])
            z_sample_2 = np.random.normal(size=[gene_size*2,latent_dim])
            z_sample_3 = np.random.normal(size=[gene_size*3,latent_dim])
       
        x_hat_1 = sess.run(x_hat_11,feed_dict = {input_z:z_sample_1})
        x_hat_2 = sess.run(x_hat_11,feed_dict = {input_z:z_sample_2})
        x_hat_3 = sess.run(x_hat_11,feed_dict = {input_z:z_sample_3})
...
#    wanna_see(data,x_hat_1)
    return z_sample_1,x_hat_1,x_hat_2,x_hat_3
#    return myx